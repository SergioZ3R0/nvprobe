...
def _run_hpcg_size(
    ...
    """Run HPCG for a single grid size."""
    try:
        if mpi_run:
...
class HpcgBenchmark(BaseBenchmark):
    ...
    def run_local(self, gpu_index: int, precision: str, batch_size: int) -> BenchmarkResult:
        ...
        for size in grid_sizes:
            result = _run_hpcg_size(binary, size, mpi_run, env, gpu_index, precision, batch_size)
            ...
            if mpi_run and ("opal_pmix" in result.error or "orte" in result.error):
                result2 = _run_hpcg_size(binary, size, None, env, gpu_index, precision, batch_size)
                if result2:
                    last_result = result2
