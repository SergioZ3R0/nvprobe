"""STREAM benchmark — measures sustainable memory bandwidth via copy/scale/add/triad."""
...
class StreamBenchmark(BaseBenchmark):
    """STREAM memory bandwidth benchmark using CuPy CUDA kernels."""
    ...
    def run_local(
        ...
        sizes = self.params.get("sizes", [10000000, 100000000])
        iterations = self.params.get("iterations", 20)

        cmd = [
            sys.executable,
            "-m",
            "nvprobe.benchmarks._cuda.stream_test",
            "--gpu",
            str(gpu_index),
            "--sizes",
...
python3 -m nvprobe.benchmarks._cuda.stream_test \\
    --gpu 0 \\
