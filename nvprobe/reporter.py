...
def generate_comparison(
    ...
    env_a = json.loads(runs_a[0].get("environment", "{}"))
    env_b = json.loads(runs_b[0].get("environment", "{}"))
...
<div class="main-content">
    <h1>{title}</h1>
    <p class="subtitle">Run: {run['name']} | {run['created_at'][:19]}</p>
    ...
    <footer>Generated by nvProbe v0.1.0 | {run['created_at'][:19]}</footer>
</div>
</body>
</html>"""
